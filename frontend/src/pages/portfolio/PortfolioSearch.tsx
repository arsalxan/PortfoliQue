...
export default function PortfolioSearch() {
  ...
  const handleSearch = async () => {
    ...
    setError('');
    try {
      const results = await portfolioService.searchPortfolios(query);
  ...
  return (
    <div className="container mt-4 mb-5">
      <div className="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
        <div>
          <h1 className="h2 fw-bold text-primary mb-1">Search Results</h1>
          <p className="text-muted mb-0">Showing results for: <span className="fw-bold">"{query}"</span></p>
        </div>
        <Link to="/portfolios" className="btn btn-outline-primary">
      ...
      {loading ? (
        ...
        <div className="text-center py-5 bg-light rounded shadow-sm">
          ...
          <Link to="/portfolios" className="btn btn-primary mt-2">Browse All Portfolios</Link>
        </div>
      ) : (
