...
public class NotificationControllerTest {
  ...
  void testGetUnreadPreview_Success() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/notifications/unread-preview"))
        .andExpect(status().isOk())
  ...
  void testMarkAllAsRead_Success() throws Exception {
    ...
    mockMvc
        .perform(put("/api/v1/notifications/mark-all-read").with(csrf()))
        .andExpect(status().isOk());
  ...
  void testMarkAsRead_Success() throws Exception {
    ...
    mockMvc.perform(put("/api/v1/notifications/1/read").with(csrf())).andExpect(status().isOk());
