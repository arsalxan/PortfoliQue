...
public class AuthControllerTest {
    ...
    void testVerifyEmail_Success() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/auth/verify-email/valid-token"))
                .andExpect(status().isOk())
    ...
    void testVerifyEmail_Failure_InvalidToken() throws Exception {
        when(authService.verifyEmail("invalid-token")).thenThrow(new RuntimeException("Invalid or expired token"));

        mockMvc.perform(get("/api/v1/auth/verify-email/invalid-token"))
                .andExpect(status().isBadRequest())
