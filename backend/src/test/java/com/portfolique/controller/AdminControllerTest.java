...
public class AdminControllerTest {
  ...
  void testGetUsers() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/admin/users").param("search", "test"))
        .andExpect(status().isOk())
  ...
  void testDeleteUser() throws Exception {
    User adminUser =
        User.builder().id(1L).username("admin").role(com.portfolique.entity.Role.ADMIN).build();
    when(userRepository.findByUsername("admin")).thenReturn(java.util.Optional.of(adminUser));

    mockMvc
        .perform(delete("/api/v1/admin/users/2").with(csrf()))
        .andDo(print())
  ...
  void testDeletePortfolio() throws Exception {
    mockMvc
        .perform(delete("/api/v1/admin/portfolios/1").with(csrf()))
        .andDo(print())
        .andExpect(status().isNoContent());
