...
public class PortfolioControllerTest {
    ...
    void testGetAllPortfolios_OpenAccess() throws Exception {
        ...
        when(portfolioService.getAllPortfolios(any(Pageable.class))).thenReturn(new PageImpl<>(List.of(res)));
    ...
    void testCreatePortfolio_Success() throws Exception {
        ...
        MockMultipartFile portfolioPart = new MockMultipartFile("portfolio", "", "application/json", objectMapper.writeValueAsBytes(req));
    ...
    void testUpdatePortfolio_ForbiddenWhenNotOwner() throws Exception {
        ...
        when(portfolioService.updatePortfolio(eq(1L), any(), any(), any(User.class)))
                .thenThrow(new ResponseStatusException(HttpStatus.FORBIDDEN, "You can only edit your own portfolio"));

        // Put request using multipart requires MockMvcRequestBuilders.multipart with HTTP method PUT
        mockMvc.perform(multipart(HttpMethod.PUT, "/api/v1/portfolios/1")
                        .file(portfolioPart)
                        .with(csrf())
                        .contentType(MediaType.MULTIPART_FORM_DATA))
                .andExpect(status().isForbidden());
